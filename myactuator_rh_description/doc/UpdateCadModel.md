...
<robot xmlns:xacro="http://www.ros.org/wiki/xacro">
  <xacro:include filename="$(find myactuator_rh_description)/urdf/myactuator_rh.xacro"/>

  <!-- Provide dimensions for specific actuator -->
  <xacro:macro name="X12_150" params="prefix joint_name simulation ifname actuator_id">
    <xacro:myactuator_rh prefix="${prefix}" joint_name="${joint_name}" simulation="${simulation}"
                          ...
                          visual="$(find myactuator_rh_description)/meshes/visual/X12_150/X12_150.dae"
...
<xacro:if value="${actuator_model == 'X12_150'}">
  <xacro:include filename="$(find myactuator_rh_description)/urdf/X12_150.xacro"/>
...
Finally we will have to also adapt the `myactuator_rh.launch.py` launch file inside this package as well as `myactuator_rh_control.launch.py` inside `myactuator_rh_bringup` adding another choice for `actuator_cmd`:
